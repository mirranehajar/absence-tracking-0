...
export class EnseignantService {
  ...
  public deleteByNumeroSOM(enseignant: Enseignant) {
    ...
    this.http.delete<number>(this._url + 'numeroSOM/' + enseignant.numeroSOM, {headers}).subscribe(
      (data) => {
        console.log(data);
  ...
  public async update() {
    // tslint:disable-next-line:max-line-length
    const headers = new HttpHeaders({ Authorization: 'Basic ' + btoa(sessionStorage.getItem('username') + ':' + sessionStorage.getItem('password')) });
    await this.http.post<Enseignant>(this._url + 'update', this.enseignantFounded, {headers}).toPromise().then(
      (data) => {
        if (data) {
          this.deleteFromList(this.enseignantFounded);
          this.enseignants.push(this.clone(data));
  ...
  public async save() {
    ...
    // tslint:disable-next-line:max-line-length
    const headers = new HttpHeaders({ Authorization: 'Basic ' + btoa(sessionStorage.getItem('username') + ':' + sessionStorage.getItem('password')) });
    await this.http.post<Enseignant>(this._url, this.enseignant, {headers}).toPromise().then(
        (data) => {
          if (data) {
            this.enseignants.push(this.clone(data));
